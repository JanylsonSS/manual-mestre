...
```mermaid
flowchart TD
    A["Início da sessão"] --> B{"Agenda tem<br/>itens vencidos?"}
    B -- "Sim" --> C["Bloco C primeiro:<br/>revisões D+1 / D+7 pendentes"]
    B -- "Não" --> D["Bloco A: teoria do<br/>capítulo do dia"]
    C --> D
    D --> E["Bloco B: prática<br/>(exercícios, código)"]
    E --> F["Ritual de fim:<br/>registrar + agendar + marcar feitos"]
    F --> G["Fim — máx. 2 capítulos novos/dia"]
```

...
## D-032 — 2026-08-05 — A condição de corrida é ensinada pela sua NÃO reprodução
**Contexto:** o roteiro previsto para o 04.21 era o clássico: rodar um contador sem trava em várias threads e mostrar o número errado. Na verificação, ele **não falhou** — nem com 4 threads, nem com 16, nem com `sys.setswitchinterval(1e-6)`, nem com a janela alargada entre leitura e escrita. Zero perdas em todas as execuções.
**Decisão:** em vez de forçar um exemplo que "sempre falha", o capítulo mostra as duas coisas: as três execuções limpas (o que o leitor obteria testando) e a mesma operação com `time.sleep(0)` entre a leitura e a escrita, que entrega a vez explicitamente e perde **75%**. O AP3 pede as duas etapas em ordem e cobra por escrito a conclusão — "o que cinco execuções sem perda provam sobre a correção?" —, cuja resposta é **nada**.
**Consequência:** o capítulo passou a ensinar algo mais valioso que o mecanismo da corrida: que existe uma classe de defeito para a qual **teste não é evidência**, e cuja confiança só vem da construção. A verificação que contrariou o roteiro virou o conteúdo central, e o mesmo raciocínio conecta com o 04.14 (tipos evitam por estrutura o que teste não pega) e com o 04.20 (o `with` garante o release por construção, não por disciplina).

## D-033 — 2026-08-06 — O projeto integrador é um pacote instalável, não um arquivo
**Contexto:** a spec pede que o 04.23 construa "um coletor concorrente de dados e refatore o Atlas para POO". A forma cômoda seria um arquivo de trezentas linhas com tudo dentro, como os demonstradores dos outros capítulos.
**Decisão:** o projeto é um pacote completo em `codigo/cap23/coletor/` — layout `src/`, `pyproject.toml` com comando de terminal, sete módulos com responsabilidades separadas (borda, domínio, coletor, tempo, registro, fonte, entrada), nove testes e `mypy --strict`. A fonte externa é simulada para o projeto rodar sem rede, com latência e taxa de falha configuráveis e `random.seed` para ser reproduzível.
**Consequência:** o módulo termina com um artefato que o leitor **instala e roda**, e que o módulo 05 continua em vez de recomeçar. Cada decisão dos vinte e dois capítulos anteriores fica visível em uma linha de código — e a fronteira do D-024 deixa de ser um princípio e vira duas pastas de arquivos, com a conversão acontecendo numa linha identificável. O custo é que o capítulo exige `pip install -e .` antes de rodar, registrado no `LEIAME.md` do projeto.

## D-034 — 2026-08-06 — Todo projeto concorrente do manual tem um teste que afirma desempenho
**Contexto:** o 04.22 mostrou que os dois erros mais comuns de asyncio produzem o mesmo sintoma — o programa fica lento e continua **correto**. Nenhum teste de correção pega isso: os resultados são os esperados, os tipos estão certos, as exceções são tratadas.
**Decisão:** o projeto do 04.23 inclui `test_coleta_concorrente_e_mais_rapida_que_sequencial`, que afirma que a versão com limite 20 é ao menos três vezes mais rápida que a com limite 1. O D1 exige o mesmo de quem refizer o projeto, e a primeira pergunta do desafio pede para introduzir um `time.sleep` e descobrir qual teste falha.
**Consequência:** o manual passa a tratar desempenho como propriedade testável em código concorrente, e não como observação manual. A margem de 3× é deliberadamente folgada — ela tolera a variação de medição que o 04.21/AP1 documentou e ainda assim falha de forma inequívoca diante de uma chamada bloqueante. É o mesmo princípio do D-032: onde o teste de correção não alcança, a estrutura do teste precisa mudar.

## D-035 — 2026-08-06 — O módulo 05 tem laboratório com PostgreSQL real, e o capítulo ensina a instalação de verdade
**Contexto:** o módulo 05 exige um servidor PostgreSQL, e instalar um é uma tarefa de administração que pode travar o leitor por uma tarde — antes de ele ver qualquer conteúdo. A alternativa cômoda seria demonstrar tudo em SQLite, o que contradiz o módulo inteiro: metade do conteúdo **é** a diferença entre arquivo e servidor.
**Decisão:** dois caminhos declarados. O capítulo 05.01 ensina a instalação real (instalador oficial no Windows, `apt` no Linux, `brew` no macOS), que é a que o leitor vai usar no trabalho; e `codigo/laboratorio.py` sobe um PostgreSQL 16 via `pgserver` para quem quiser rodar os exemplos hoje. Os dois usam a mesma variável `AURORA_URI`, então o mesmo código roda nos dois. Os dados carregados são **idênticos aos do módulo 03**.
**Consequência:** todo número e toda saída do módulo 05 são execução real contra um Postgres de verdade — planos de `EXPLAIN` (05.11), `JSONB` (05.03), MVCC (05.01) —, e não descrições. E a igualdade dos dados com o módulo 03 transforma cada capítulo numa comparação direta: o leitor roda a mesma consulta nos dois bancos e vê o que muda. O custo é uma dependência a mais no laboratório, isolada num arquivo que o resto do módulo importa por uma função.

## D-036 — 2026-08-06 — O laboratório deixa o servidor de pé depois de sair
**Contexto:** o `laboratorio.py` do 05.01 imprimia `export AURORA_URI=...` com a instrução "para usar em outro terminal". Ao escrever o 05.02, que é inteiro sobre o `psql`, a instrução se revelou falsa: o `psql` respondia `connection to server on socket ".s.PGSQL.5432" failed: No such file or directory`. O motivo está no código do `pgserver` — `atexit.register(self._cleanup)` com `cleanup_mode='stop'` por padrão, que derruba o servidor quando o processo que o abriu termina.
**Decisão:** `uri()` passa a chamar `get_server(..., cleanup_mode=None)`, e o script ganha `--parar` (derruba preservando os dados) e `--bin` (imprime a pasta com `psql`, `pg_dump` e afins, que vem dentro do próprio pacote pip). O motivo fica registrado em comentário no código, com a mensagem de erro original.
**Consequência:** o 05.02 pode ser estudado como ele se usa de verdade — um terminal com o servidor, outro com o `psql` —, e quem não instalou o PostgreSQL ainda assim tem um `psql` 16.2 real. O custo é que o servidor sobrevive à sessão e precisa ser derrubado de propósito, o que é o comportamento de um servidor e está declarado na saída do script.

## D-037 — 2026-08-06 — O MongoDB do módulo roda localmente, e não num Atlas
**Contexto:** os capítulos 05.12–05.14 precisam de um MongoDB real. O sandbox de geração não tem `mongod`, e os substitutos em Python (`montydb`, `mongita`) fazem CRUD e filtros mas **não implementam `aggregate`** — que é o assunto do 05.14. Um cluster Atlas gratuito foi criado e não resolveu: o proxy do ambiente encaminha apenas HTTPS/443, e o TLS na porta 27017 falhava com `TLSV1_ALERT_INTERNAL_ERROR`, com credenciais válidas.
**Decisão:** não sair da máquina. O MongoDB 8.0.4 é baixado do repositório oficial (que responde na 443) e roda localmente, como replica set de um nó — a mesma estratégia que o `pgserver` resolveu para o Postgres no D-035. Verificado antes de escrever qualquer capítulo: `aggregate` com `$group`/`$sum`/`$multiply`/`$sort`, e transação entre coleções com `rollback` devolvendo o estado anterior.
**Consequência:** os três capítulos de MongoDB saem verificados como todo o resto do manual, sem marcador de pendência e sem depender de conta em serviço pago. O replica set de um nó, que existia só para satisfazer o requisito das transações, virou conteúdo: ele é o que permite ao 05.12 comparar Postgres e Mongo com as duas saídas na tela. A limitação declarada é `$search`, que é do Atlas e não do MongoDB — ele não roda em servidor local nenhum, e fica fora do módulo.

## D-038 — 2026-08-06 — A SQL injection do 05.04 é demonstrada destruindo uma tabela de verdade
**Contexto:** a spec pede que o 05.04 "explique SQL injection". A forma segura seria descrever o ataque e mostrar a consulta resultante em texto. O roteiro original fazia isso e previa que o terceiro ataque — `x'; DROP TABLE ...; --` — levantaria exceção no driver.
**Decisão:** executar os três ataques contra uma tabela criada pelo próprio script. A previsão estava errada: o `DROP` funcionou, sem exceção, e derrubou as cinco cenas seguintes. Em vez de contornar, o achado virou a §7 do capítulo (os dois protocolos do PostgreSQL) e a explicação de por que parametrizar é defesa estrutural e escapar aspas não é.
**Consequência:** o capítulo tem a única demonstração do manual em que o dano acontece na tela — `a tabela ainda existe? NÃO — foi destruída` —, e a explicação que a acompanha é mecânica, não moral. O script recria a tabela logo depois, com comentário dizendo por quê. O código vulnerável carrega `# noqa: S608` e o capítulo usa isso para apresentar a regra do `bandit`/`ruff`, transformando a linha proibida no argumento a favor de ligar o analisador no CI.

## D-039 — 2026-08-06 — O laboratório usa colunas de identidade, e ressincroniza as sequências
**Contexto:** o `laboratorio.py` criava as tabelas com `id integer PRIMARY KEY`, sem geração automática — o módulo 03 carregava ids explícitos e nunca precisou de mais. Ao escrever o 05.07, o primeiro `INSERT` feito pelo ORM falhou com `null value in column "id" of relation "pedidos"`: o modelo do 05.06 declara `Mapped[int]` com `primary_key=True`, o que gera `SERIAL`, e assume que o banco produz o valor.
**Decisão:** as quatro tabelas passam a usar `GENERATED BY DEFAULT AS IDENTITY`, que aceita id explícito na carga **e** gera quando ele falta. E, como carregar com id explícito não avança a sequência, a carga termina com um `setval` por tabela — com comentário explicando que este é o defeito clássico de restaurar um dump.
**Consequência:** o ORM passa a criar linhas normalmente, e o laboratório ganhou um comportamento que o SQLite do módulo 03 já tinha (`INTEGER PRIMARY KEY` autoincrementa), o que **aumenta** a paridade em vez de reduzi-la. E o episódio virou conteúdo em dois lugares: a §10 do 05.07 documenta a falha, e o gabarito do 05.06/AP2 usa esse caso para listar o que a conferência modelo-contra-banco **não** detecta — ela compara nomes e tipos, e não a existência de sequência.

## D-040 — 2026-08-06 — Toda medição do bloco ORM reporta consultas, e não só tempo
**Contexto:** o bloco 05.05–05.09 é sobre custo, e a tentação é medir tempo. Tempo varia com a máquina, o cache e a carga: os 660 ms do N+1 viram 200 ms numa máquina rápida.
**Decisão:** todo instrumento do bloco (`gravar_sql` no 05.07, `medir` no 05.09) reporta **contagem de consultas** ao lado do tempo, e os exercícios cobram a contagem. O 05.09/§10 explicita o critério: a contagem é um invariante e o tempo não é, e por isso só a primeira serve para um `assert`.
**Consequência:** os capítulos ficam reprodutíveis — as contagens de quem repetir precisam bater exatamente, e discrepância significa código diferente, não máquina diferente. Isso torna os exercícios verificáveis pelo próprio estudante, e é o que permite ao 05.08/MP e ao 05.09/D1 pedirem testes que afirmam orçamento de consultas, no mesmo espírito do D-034.

## D-041 — 2026-08-06 — Os modelos ORM descrevem as tabelas que já existem, e não um schema novo
**Contexto:** seria mais cômodo inventar um schema limpo para o bloco ORM, com colunas convenientes para cada demonstração.
**Decisão:** `codigo/modelo.py` descreve exatamente as quatro tabelas do `laboratorio.py`, que são as do módulo 03. A única tabela nova é `Cotacao`, declarada como "existe só para o 05.06" e usada para demonstrar `create_all` — e `Etiqueta` mais `produto_etiqueta`, criadas e destruídas pelo próprio script do 05.08.
**Consequência:** o exercício que o estudante faz é o que ele vai fazer no trabalho — escrever modelos para um banco que já existe —, e a cena 6 do 05.06 pode conferir modelo contra banco de verdade, o que virou o instrumento que encontrou a divergência de `timestamptz`. O custo é que algumas demonstrações precisaram de dados semeados (05.09) em vez de tabelas sob medida.
